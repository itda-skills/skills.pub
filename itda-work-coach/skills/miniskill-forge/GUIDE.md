...
> 이 가이드는 miniskill-forge 한 가지를 어떻게 쓰는지에 집중합니다. itda-work-coach 전체가 처음이라면 플러그인 `GUIDE.md`(시작 가이드)를 먼저 보세요.
...
```
/miniskill-forge 이거 매번 자동으로 했으면 좋겠어. 한 번 만들어두고 부를 수 있게 해줘
```

```
/miniskill-forge 내 반복 업무를 스킬로 만들어줘
```

```
/miniskill-forge 맨날 같은 프롬프트 다시 쓰기 싫어. 이 작업 스킬로 등록해줘
```

질문에 답만 하면 됩니다. 코치가 직군에 가까운 예시를 보여주고, 붙여넣을 프롬프트를 같이 만들고, 마지막에 미니스킬 등록·호출까지 챙겨 줍니다.

> ⚠️ **무엇을 자동화할지 아직 막막하면** 이 스킬이 아닙니다. "뭘 자동화하면 좋을지 같이 골라줘"라고 말하면 코치가 **발굴 단계(work-find)**로 먼저 안내합니다.
...
- 참고할 자료는 워크스페이스 폴더에 **파일로** 둡니다. (워크스페이스가 뭔지 모르면 플러그인 시작 가이드 `itda-work-coach/GUIDE.md`를 먼저 보세요.)
...
```
/miniskill-forge 회의록을 매번 같은 양식으로 정리하는 걸 미니스킬로 만들고 싶어
```
...
```
/miniskill-forge 워크스페이스의 0612_주간회의.txt로 회의록 정리해줘
```
...
- ⚠️ **무엇을 자동화할지 정하는 단계가 아닙니다.** 굳힐 작업이 이미 정해졌을 때 쓰세요. 아직 막막하면 발굴부터 — 코치에게 "뭘 자동화하면 좋을지 같이 골라줘"라고 말하면 **work-find**로 안내합니다. "굳힐 건 정했는데 오늘 크기가 막막"하면 **hour-slice**로 한 조각부터 시작합니다.
