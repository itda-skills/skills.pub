...
| [`flight-search`](skills/flight-search/SKILL.md) | Google Flights 공개 검색으로 항공권을 조회·비교하는 스킬입니다. |
...
| [`place-finder`](skills/place-finder/SKILL.md) | 카카오맵 기준으로 근처 장소를 목적별로 찾아주는 스킬입니다. |
...
```bash
claude plugin install itda-skills/skills.pub itda-travel-plan
```
...
just test itda-travel-plan

# 또는 플러그인 디렉토리 로드
claude --plugin-dir itda-travel-plan
```

> train-ktx·train-srt이 같은 모듈명(`stations` 등)을 써서, `pytest itda-travel-plan`
> 로 전체를 한 번에 실행하면 모듈 캐시가 충돌합니다. `just test`(및 CI)는 스킬별로
> 분리 실행하므로 안전합니다. 단일 스킬은 `just test-skill itda-travel-plan <skill>`.
