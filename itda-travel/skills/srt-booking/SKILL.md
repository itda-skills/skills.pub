...
description: >
  ...
  "아까 그 열차로 예약해줘"처럼 말하면 됩니다. SR 비공식 API를 사용하며,
...
SRT(수서고속철) 열차를 **검색**하고 **예약**합니다. KTX는 자매 스킬 ktx-booking
...
> 러너는 자동 처리합니다. 로컬 직접 실행 시 저장소 루트에서 `PYTHONPATH=shared`
> 를 앞에 붙입니다.

```bash
# macOS/Linux (저장소 루트 기준)
PYTHONPATH=shared python3 itda-travel/skills/srt-booking/scripts/main.py search --dep 수서 --arr 부산 --date 20260612 --time 180000
PYTHONPATH=shared python3 itda-travel/skills/srt-booking/scripts/main.py reserve --dep 수서 --arr 부산 --date 20260612 --time 180000 --index 0            # 미리보기(예약 안 함)
PYTHONPATH=shared python3 itda-travel/skills/srt-booking/scripts/main.py reserve --dep 수서 --arr 부산 --date 20260612 --time 180000 --index 0 --confirm  # 실제 예약
PYTHONPATH=shared python3 itda-travel/skills/srt-booking/scripts/main.py reservations

# Windows
$env:PYTHONPATH="shared"; py -3 itda-travel/skills/srt-booking/scripts/main.py search --dep 수서 --arr 부산
```
...
구간이면 ktx-booking 사용을 권합니다. SRT는 수서·동탄·평택지제에서 출발합니다.
...
- **KTX/Korail** — 대상 아님(서울·용산·광명 출발은 ktx-booking). SRT 전용.
