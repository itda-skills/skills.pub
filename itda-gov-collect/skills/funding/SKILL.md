...
- **HWP/HWPX** → `Skill` 도구로 `itda-content-create:hwpx` 를 호출해 마크다운 변환(표 플래튼 포함)
- **PDF** → `Skill` 도구로 `itda-knowledge-build:pdf-context-refinery` 를 호출해 본문 추출

> **크로스플러그인 미설치 계약 (조용한 생략 금지)**: 이 두 스킬은 **itda-content-create(hwpx)·itda-knowledge-build(pdf-context-refinery)** 플러그인 소속이고
> funding 은 **itda-gov-collect** 소속이라 함께 설치돼 있지 않을 수 있다. 호출이 실패하거나 스킬이 없으면
> **변환을 조용히 건너뛰지 않는다** — 해당 공고를 "첨부 원문 미변환" 으로 표시하고, 보고서 한계 고지에
> **"HWP/PDF 첨부 N건 미변환 — itda-content-create·itda-knowledge-build 플러그인을 설치하면 본문까지 검증할 수 있습니다"** 를 명시한다.
