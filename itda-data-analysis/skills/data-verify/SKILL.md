...
itda-data-analysis 데이터 양심 vertical(정리 `data-prep` · 질문 `data-ask` · 수식감사 `data-audit` ·
...
```
data-verify(검수 보고) → itda-data-analysis:data-auditor(원장·원 셀 독립 재계산, AUDIT_SCHEMA JSON)
```

`Agent` 도구로 `itda-data-analysis:data-auditor` 를 명시 디스패치한다(자동 위임은 발동하지 않는다). 프롬프트에
