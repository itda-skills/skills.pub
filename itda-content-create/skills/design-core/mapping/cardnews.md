...
design-core 토큰을 정사각/세로형 카드뉴스 이미지(인스타·블로그용)로 렌더한다. PPTX 와 달리 **Pillow/SVG 레이아웃 합성**(텍스트+도형+모티프)이 주 경로이며, AI 삽화가 필요하면 `itda-content-create:imagegen` 을 삽화 소스로 선택 연계한다(색 정체성은 design-core 가, 그림은 imagegen 이).
...
- `itda-content-create:imagegen` 핸드오프(팔레트→삽화 톤) 계약.
