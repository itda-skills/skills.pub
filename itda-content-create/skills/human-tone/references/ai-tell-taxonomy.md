...
- 처방: **남발(문서 내 3회+ 반복)만 억제** — "~로", "~해서", "~함으로써" 등으로 분산하되 **한두 번은 보존**한다. 1~2회 사용을 지우는 것은 과교정이다.
- 근거: 상류 empirical-validation(G²) — 2026-09 반영. 최희경(2016) 비번역 84.4 vs 번역 42.1(1000어절당) — **원어민이 두 배 더 쓴다**. 자체 AI 코퍼스(60편)에도 거의 미출현. 상세는 `references/empirical-validation.md`.
- 정정 이력: v2.0 까지 `[S1]`·"무조건 분산"이었다(2026-05 차용 당시 상류 v1.6.1 원문). 2026-09-01 #1619 에서 상류 반증(기각)을 반영해 S2 로 강등하고 처방을 조건부로 바꿨다. 조용히 고치지 않고 여기 남긴다.
...
### A-16. 영어 대명사 직역 (그/그녀/그것/그들) [S1 · 번역 맥락 전용] · v2.0 신규
- **적용 범위: 번역 맥락 전용 — 자생 한국어 산문에서는 발동하지 않음.** 영어 원문을 번역·요약한 텍스트(또는 `--scene translation`)에서만 검출한다. 자생 한국어 산문의 `그는/그의`는 정상 응결 장치이며 지우면 과교정이다.
- 근거: 상류 empirical-validation(G²) — 2026-09 반영. 자생 산문 코퍼스에서 `그/그의` 는 **사람 1.9 vs AI 0.0**(1000어절당), 200자 창 3회+ 군집도 사람 3.9% vs AI 0.7% — 사람이 더 쓴다. 이 측정은 자생 산문끼리의 비교라 번역 맥락은 사정거리 밖이며, 그래서 기각이 아니라 **범위 한정**이다. 상세는 `references/empirical-validation.md`.
- 정정 이력: v2.0 도입 당시 적용 범위 조건 없이 `[S1]`이었다. 2026-09-01 #1619 에서 "번역 맥락 전용" 을 명시했다.
...
### I-1. "것이다" 종결 남발 [S2 · 조건부]
- "~한 것이다", "~일 것이다"가 문단의 대표 종결.
- 처방: **연속 3회+ 남발일 때만** 일부를 확정 서술("~다")로 평서화. 단발·산발 사용은 보존한다 — 무조건 평서화는 과교정이다.
- 근거: 상류 empirical-validation(G²) — 2026-09 반영. `~것이다` 정밀 측정 AI 20.4 vs 사람 43.0(1000문장당, G²=6.2) — **사람이 두 배**. 대안 가설(단락 말 위치·연속 반복)도 무너졌다(단락 말 0.3배, 연속 반복 AI 0.0). 상류의 최초 측정은 `이다` 종결을 통째로 세는 오류(AI 137.0 vs 사람 142.1)가 있었고 상류가 정정 기록을 남겼다 — 방향은 같으나 수치를 인용할 때 정밀 측정값을 쓴다. 상세는 `references/empirical-validation.md`.
- 정정 이력: v2.0 까지 "확정 서술로. ~다로 끝맺음"(무조건)이었다. 2026-09-01 #1619 에서 상류 반증(기각)을 반영해 조건부로 바꿨다.
...
- **v2.1** (2026-09-01, #1619): **상류 경험적 반증 반영 3건 — 정정 이력을 각 항목에 남김**. `epoko77-ai/im-not-ai` 의 `empirical-validation.md`(AI 60편 vs 2022년 이전 사람 60편, 로그우도비 G²)가 본진 규칙 2건을 기각하고 1건의 범위를 한정했다:
  - `A-2` "~를 통해" — S1 → **S2 강등**, 남발(3회+)만 억제·한두 번 보존(최희경 2016 원어민 2배)
  - `I-1` "것이다" 종결 — 무조건 평서화 기각 → **연속 3회+ 남발일 때만**(AI 20.4 vs 사람 43.0, G²=6.2)
  - `A-16` 영어 대명사 직역 — 기각 아님, **번역 맥락 전용**으로 범위 한정(자생 산문 사람 1.9 vs AI 0.0)
  - 동반: `scripts/verify_gates.py` 신설(P0 문자 변경률·P1 목표 달성/과교정·P2 수사 전멸·P3 보존 불변식 — 30/50% 문서 규율을 스크립트 판정으로). 근거 요약은 `references/empirical-validation.md`, 어트리뷰션은 `README.md` 차용 출처.
...
## K. 직장인 사무 한국어 특화 패턴 (itda-work 확장)

> 이 섹션은 itda-work/human-tone에서 추가. ref-im-not-ai v1.6 분류학(A~J)이 한국어 출판·저널리즘·블로그 일반에 최적화되어 있는 데 반해, 본 K 카테고리는 사내 보고서·이메일·기획서·공지 등 직장인 사무 한국어에 특화된 AI 슬롭 패턴을 다룬다. 일반 분류학으로 포착되지 않는 사무 도메인 고유의 군더더기·과공손·차용어 인플레이션·ChatGPT 흔적을 별도 신설했다. 모든 항목은 기존 스키마(id/category/name/severity/definition/signature_example/suggested_fix/threshold)에 `domain_note` 필드를 추가한 형태다.

### K-1. 보고서식 군더더기 (Reportese Padding)

- **id**: K-1
- **category**: K
- **severity**: S2
- **definition**: 추측·수동·피동·명사화를 중첩해 단정 책임을 회피하면서 분량만 부풀리는 사무 보고서 특유의 결구.
- **signature_example**:
  - "고객 이탈이 증가한 측면이 있는 것으로 보여집니다."
  - "조직 개편에 대한 방안 마련이 필요할 것으로 사료됩니다."
  - "전반적으로 매출이 하락하는 양상을 띠고 있습니다."
  - "프로세스 개선을 검토할 필요가 있다고 판단됨."
- **suggested_fix**: 행위 주체와 시점을 명시한 능동 단문으로 압축. "고객 이탈이 3월부터 늘었다.", "조직 개편안이 필요하다.", "매출이 1분기 12% 줄었다.", "프로세스를 4월까지 개선하자."
- **domain_note**: 보고서·임원 보고·주간 업무 보고에서 빈도 최대. 이메일·공지에는 약함.
- **threshold**: 단락(≈3문장)당 "보여집니다 / 사료됩니다 / 양상 / 판단됨 / 측면이 있" 표지 2회+, 또는 한 문장 안에 추측·수동·피동 동시 누적.

### K-2. 이메일 과공손 (Email Over-Politeness)

- **id**: K-2
- **category**: K
- **severity**: S2
- **definition**: 한 메일 안에서 격식 표현·사과 클리셰·부탁 결구가 반복 누적되어 본론 가독성과 우선순위가 묻히는 패턴.
- **signature_example**:
  - "다름이 아니오라, 바쁘신 와중에 죄송하지만 …"
  - 본문에 "확인 부탁드립니다" 4회+
  - "검토해 주시면 감사하겠습니다 / 조치해 주시면 감사하겠습니다 / 회신해 주시면 감사하겠습니다" 3개+ 체이닝
  - 결구 고정 클리셰 "검토 후 회신 부탁드립니다."
- **suggested_fix**: 인사 1회·부탁 1회·결구 1회로 압축, 마감일·응답 형식 한 줄 명시. 예: "5/15(목) 14시까지 첨부 양식으로 회신 부탁드립니다."
- **domain_note**: 이메일·메신저 장문 요청·외부 협력사 커뮤니케이션. 보고서·기획서에는 거의 없음.
- **threshold**: (a) 두 도입 클리셰 동시 출현, (b) "확인 부탁드립니다" 4회+, (c) "~해 주시면 감사하겠습니다" 3회+ 중 하나.

### K-3. 기획서 추상명사·차용어 인플레이션 (Buzzword Inflation)

- **id**: K-3
- **category**: K
- **severity**: S2
- **definition**: 사내 기획서·전략 문서에서 의미가 비어 있는 영어 차용 추상어를 단락당 다수 동원해, 구체 행동·지표·일정 부재를 위장하는 패턴. B(영어 인용)는 출판물의 단어 인용을 다루고, K-3은 사무 기획서 특유의 사내 차용어 군집에 집중.
- **signature_example**: "이번 분기에는 시너지를 극대화하고, 밸류 체인 전반에서 애자일하게 움직이며, 유저 센트릭 관점으로 엔드 투 엔드 오너십을 가지고 임팩트 있는 결과를 만들겠습니다."
- **suggested_fix**: 차용어를 (a) 한국어 일상어 또는 (b) 측정 가능한 지표·기한·담당으로 치환. "시너지 극대화" → "두 팀이 5월까지 공동 KPI 1개 합의", "임팩트 있는 결과" → "월 활성 사용자 +5%."
- **domain_note**: 기획서·전략 보고·OKR/KPI 문서. 공지·간단 안내문에서는 임계 미달.
- **threshold**: 단락당 다음 어휘군 4개+ 출현 시 트리거 — `시너지 / 밸류 체인 / 애자일하게 / 유저 센트릭 / 엔드 투 엔드 / 오너십을 가지고 / 임팩트 있는 / 디지털 트랜스포메이션 / 인사이트 / 페인포인트`. 3개 이하라도 같은 어휘 문서 5회+ 반복 시 보조 트리거.

### K-4. 공지·안내문의 ChatGPT 흔적 (LLM-Telltale in Announcements)

- **id**: K-4
- **category**: K
- **severity**: S1
- **definition**: 사내 공지·안내문에서 LLM이 사용자 프롬프트에 응답하던 흔적이 다듬어지지 않고 남은 패턴. 잉여 자기소개·메타 발화·구조 단조로움·결구 부재가 동시 출현 시 신호 강도 최대.
- **signature_example**:
  - "안녕하세요, 저는 인사팀입니다. …" (사내 헤더에 이미 표기, 잉여)
  - "이 내용은 5월 보안 교육 안내를 위해 작성되었습니다." (메타 발화)
  - 본문 5단락 모두 글머리 기호 단독 구성
  - 결구 "감사합니다" 누락 또는 매 단락마다 반복
- **suggested_fix**: (a) 자기소개·메타 발화 행 삭제, (b) 글머리 기호는 핵심 항목 1개 단락에만, (c) 결구는 문서 끝 1회로 통일.
- **domain_note**: 사내 공지·전사 안내·HR/IT 안내문 강함. 보고서·기획서에서는 (c) 신호만 동일 적용.
- **threshold**: 다음 중 2개 이상 — (a) "안녕하세요, 저는 …입니다" 자기소개, (b) "이 내용은 ~를 위해 작성되었습니다" 메타 발화, (c) 본문 단락의 80%+ 글머리 단독, (d) "감사합니다" 0회 또는 3회+.

### K-5. 결론 회피 결구 (Hedged Conclusion)

- **id**: K-5
- **category**: K
- **severity**: S3
- **definition**: 보고서·이메일 마지막 단락에서 결정·요청·권고 없이 "~할 수 있을 것으로 기대된다 / 향후 추이를 지켜볼 필요가 있다 / 지속적인 모니터링이 요구된다" 류로 끝맺어 의사결정 비용을 수신자에게 떠넘기는 패턴.
- **signature_example**:
  - "향후 시장 추이를 지속적으로 모니터링할 필요가 있을 것으로 사료됩니다."
  - "관련 부서와의 협의를 통해 개선이 이루어질 수 있을 것으로 기대됩니다."
- **suggested_fix**: "누가 / 언제까지 / 무엇을 / 어떤 기준으로" 결정·실행할지 한 문장으로 명시. 예: "영업팀이 5/20까지 단가 인하안 1개를 제출한다."
- **domain_note**: 보고서·임원 보고 결론부, 이메일 결구.
- **threshold**: 마지막 1~2단락에 "기대된다 / 모니터링 / 지켜볼 필요 / 검토가 요구된다" 표지 + 동일 단락에 주체·시점·지표 모두 부재.

### K 카테고리 적용 우선순위

K-4(S1) > K-1·K-2·K-3(S2) > K-5(S3). K-4는 LLM 흔적이 외부 협력사·전사 수신자에게 노출될 위험이 가장 크므로 항상 1순위.
