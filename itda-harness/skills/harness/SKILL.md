...
name: harness
description: >
  하네스를 구성합니다. 전문 에이전트를 정의하고, 그 에이전트가 사용할 스킬을 생성하는 메타 스킬입니다.
  "하네스 구성해줘", "하네스 구축해줘", "하네스 설계 도와줘"처럼 말하면 됩니다.
...
| 모드 | 언제 사용 | 특성 |
|------|----------|------|
| **에이전트 팀** (기본) | 2명 이상 협업, 실시간 조율·피드백 교환이 필요, 중간 산출물 상호 참조 | `Agent`(이름 지정 병렬 스폰) + `SendMessage` + `TaskCreate`로 자체 조율 |
...
> **Workflow 도구가 있는 환경(Claude Code):** 항목 수가 많고 제어 흐름이 결정론적인 fan-out(수십 파일 마이그레이션, 찾기→검증 파이프라인 등)은 에이전트 팀 대신 Workflow 스크립트(`agent()`/`parallel()`/`pipeline()`, 재개·토큰 예산 지원)가 적합할 수 있다. 단 Workflow는 사용자가 멀티에이전트 오케스트레이션을 명시적으로 요청했을 때만 사용하고, 미지원 환경에서는 위 3개 모드로 구성한다.
...
**모델 설정:** 에이전트는 세션 모델을 상속하는 것이 기본이다 — Agent 도구 호출 시 `model` 파라미터를 생략한다. 특정 단계에 더 높은 추론 품질이 필요하다고 확신할 때만 호출 시점에 `model`을 명시적으로 지정한다.

**팀 재구성:** 팀은 세션마다 하나 존재하는 암묵 팀이며 별도 생성·해체 절차가 없다. 파이프라인 패턴처럼 Phase별로 다른 전문가 조합이 필요하면, 이전 팀원들의 작업 완료를 확인하고 산출물을 파일로 저장한 뒤 다음 Phase의 팀원들을 새 `Agent` 호출로 스폰한다. 완료된 팀원의 이름은 계속 유효하므로, 이전 컨텍스트가 필요하면 `SendMessage`로 이어서 지시할 수 있다.
...
**에이전트 팀 패턴 (기본):**
오케스트레이터가 `Agent` 도구로 팀원들을 이름(`name`) 지정해 병렬 스폰한다 — 스폰된 에이전트들은 세션의 단일 암묵 팀에 소속되며 별도 팀 생성 절차는 없다. `TaskCreate`로 작업을 할당하고, 팀원들은 `SendMessage`로 직접 통신하며 자체 조율한다. 리더(오케스트레이터)는 진행 상황을 모니터링하고 결과를 종합한다.

```
[오케스트레이터/리더]
    ├── Agent(name: "teammate-N", prompt) ×N — 단일 메시지에서 병렬 스폰 (암묵 팀 소속)
...
- **Phase 간 팀 재구성**: Phase마다 이전 팀원들의 작업 완료를 확인한 뒤 다음 Phase 팀원들을 새로 스폰, 사이에 서브 에이전트 호출 삽입
...
- [ ] Agent 호출의 `model` 파라미터는 기본 생략(세션 상속) — 명시한 곳은 상향 사유가 있는지 확인
