...
세션에는 리더(메인 에이전트)를 중심으로 한 **단일 암묵 팀**이 존재한다 — 별도 팀 생성 도구는 없다. 리더가 `Agent` 도구로 팀원들을 이름(`name`) 지정해 스폰하면, 팀원들은 독립 인스턴스로 실행되며 그 팀에 소속된다. 팀원들은 `SendMessage`로 직접 통신하고, 공유 작업 목록(`TaskCreate`/`TaskUpdate`)으로 자체 조율한다.
...
**핵심 도구:**
- `Agent(name, prompt, subagent_type)`: 팀원 스폰 — `name`을 지정해야 SendMessage로 주소 지정 가능. 단일 메시지에서 여러 호출을 묶으면 병렬 스폰
- `SendMessage({to: name})`: 특정 팀원에게 메시지 — 완료된 팀원의 이름도 계속 유효하며, 전송하면 해당 컨텍스트를 유지한 채 재개된다
...
- 백그라운드 팀원이 완료되면 자동으로 리더에게 알림
- 계획 승인 모드(`mode: "plan"`)로 위험한 작업 전 검토 가능

**제약:**
- 팀은 세션당 하나의 암묵 팀 — 생성·해체 개념이 없고, 팀원 구성은 스폰으로 자유롭게 확장
- 중첩 팀 불가 (팀원이 자신의 팀을 만들 수 없음)
- 리더 고정 (메인 세션이 리더, 이전 불가)
- 토큰 비용 높음

**팀 재구성 패턴:**
Phase별로 다른 전문가 조합이 필요하면, 이전 팀원들의 산출물을 파일로 저장하고 작업 완료를 확인한 뒤 다음 Phase의 팀원들을 새 `Agent` 호출로 스폰한다 — 해체 절차는 없다. 이전 팀원의 산출물은 `_workspace/` 에 보존되므로 새 팀원이 Read로 접근 가능하고, 이전 팀원의 컨텍스트가 필요하면 `SendMessage`로 이어받는다.
...
### Workflow 도구 — 결정론적 오케스트레이션 (지원 환경 한정)

Claude Code처럼 `Workflow` 도구가 있는 환경에서는 제3의 선택지가 있다: 오케스트레이션을 LLM 판단이 아닌 **스크립트**(`agent()`/`parallel()`/`pipeline()`)로 결정론적으로 기술한다.

**적합한 경우:**
- 항목 수가 많은 fan-out — 수십 파일 마이그레이션, 찾기→검증 파이프라인, 대량 감사
- 제어 흐름이 결정적 — 루프, 조건 분기, 단계 의존이 코드로 표현 가능
- 중단 후 재개(resume)·토큰 예산(budget) 제어가 필요

**에이전트 팀이 여전히 나은 경우:** 팀원 간 실시간 토론·합의·상호 도전이 품질을 좌우하는 작업 (Workflow 의 에이전트들은 서로 통신하지 않는다).

**주의:** Workflow는 다수 에이전트를 스폰해 토큰을 크게 소비하므로, 사용자가 멀티에이전트 오케스트레이션을 명시적으로 요청(옵트인)했을 때만 사용한다. 미지원 환경(Cowork 등)에서는 에이전트 팀/서브 에이전트로 구성한다.
...
**모델:** 세션 모델 상속이 기본 — Agent 도구 호출 시 `model` 파라미터를 생략한다. 특정 단계에 더 높은 추론 품질이 필요하다고 확신할 때만 명시적으로 지정한다.
