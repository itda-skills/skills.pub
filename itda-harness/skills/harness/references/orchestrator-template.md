...
2명 이상의 에이전트가 협업할 때 **가장 먼저 검토하는 기본 모드**. `Agent` 도구로 팀원들을 이름 지정해 병렬 스폰하고(세션의 암묵 팀에 소속), 공유 작업 목록과 `SendMessage`로 조율한다.
...
1. 팀원 스폰 — 단일 메시지에서 N개 `Agent` 호출을 동시에 실행한다 (병렬 스폰, 세션의 암묵 팀에 소속):
   ```
   Agent(name: "{teammate-1}", subagent_type: "{type}", prompt: "{역할 설명 및 작업 지시}")
   Agent(name: "{teammate-2}", subagent_type: "{type}", prompt: "{역할 설명 및 작업 지시}")
   ...
   ```

   > `name`을 지정해야 이후 `SendMessage({to: name})`로 주소 지정이 가능하다. 모델은 세션 상속이 기본 — 특정 팀원에게 상향이 필요할 때만 `model` 파라미터를 명시한다.
...
### Phase 5: 정리
1. 모든 팀원의 작업 완료 확인 (완료 알림 수신 또는 TaskGet)
2. `_workspace/` 디렉토리 보존 (중간 산출물은 삭제하지 않음 — 사후 검증·감사 추적용)
3. 사용자에게 결과 요약 보고

> **팀 재구성이 필요한 경우:** Phase별로 다른 전문가 조합이 필요하면, 현재 팀원들의 작업 완료를 확인한 뒤 다음 Phase의 팀원들을 새 `Agent` 호출로 스폰한다 — 암묵 팀은 세션과 함께 상시 존재하므로 해체·재생성 절차가 없다. 이전 팀원의 산출물은 `_workspace/`에 보존되므로 새 팀원이 Read로 접근 가능하고, 완료된 팀원의 이름은 계속 유효해 필요 시 `SendMessage`로 컨텍스트를 이어받는다.

## 데이터 흐름

```
[리더] → Agent 병렬 스폰 → [teammate-1] ←SendMessage→ [teammate-2]
                          │                           │
                          ↓                           ↓
...
|---------|------|------|-------|-------------------|
| {agent-1} | {소스} | `_workspace/{phase}_{agent}_{artifact}.md` | (세션 상속) | true |
| {agent-2} | {소스} | `_workspace/{phase}_{agent}_{artifact}.md` | (세션 상속) | true |
...
1. `Agent` 병렬 스폰으로 통합 팀 구성 (editor + fact-checker + synthesizer)
...
**하이브리드 전환 규칙:**
- 팀 → 서브: 팀원들의 작업 완료를 확인한 후 서브 에이전트 호출 (팀 해체 절차 불요)
- 서브 → 팀: 서브 에이전트의 파일 산출물을 팀원들에게 Read 경로로 전달
- 팀 → 팀: 다음 Phase 팀원들을 새 `Agent` 호출로 스폰 (암묵 팀은 세션당 하나로 상시 존재 — 구 "세션당 1팀만 활성" 제약은 폐지)
...
2. **팀 모드는 Agent 스폰/SendMessage/TaskCreate 사용법을 구체적으로** — 팀원 스폰(이름 지정), 작업 등록, 통신 규칙
...
준비 → Phase 0(컨텍스트 확인) → Agent 병렬 스폰 + TaskCreate → N개 팀원 병렬 실행 → Read + 통합 → 정리.
