...
description: >
  ...
  [책임 경계] 본 스킬은 R-ONE 가격지수와 파생 통계 전담 — 실거래 원본 행 수집은 itda-realty-data:realty-deals(derive 의 입력), 공급·청약 지표는 itda-realty-data:realty-supply.
license: Apache-2.0
...
| 상황 | 대신 쓸 스킬 |
|---|---|
| 실거래 개별 건 원본(CSV/JSON) | itda-realty-data:realty-deals |
| 미분양·인허가·청약 | itda-realty-data:realty-supply |
| 전세가율·갭 | itda-realty-data:realty-jeonse-gap |
| 스킬팩 전체 안내·키 발급 | itda-realty-data:realty-meta |

## 테스트 실행

```bash
# macOS/Linux
python3 -m pytest itda-realty-data/skills/realty-price-stats/tests/ -v

# Windows
py -3 -m pytest itda-realty-data/skills/realty-price-stats/tests/ -v
```
