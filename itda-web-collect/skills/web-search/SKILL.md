...
description: >
  ...
  다루지 않습니다.
  [책임 경계] 본 스킬은 검색엔진 결과 목록(제목·URL·발췌) 수집 전담 — 찾은 URL 의 본문 추출은 itda-web-collect:web-reader, 네이버 블로그는 itda-web-collect:blog-reader 이며, 시장조사 보고서·팩트체크는 다루지 않습니다.
license: Apache-2.0
...
| 상황 | 대신 쓸 스킬 |
|---|---|
| 찾은 URL 의 본문을 마크다운으로 추출 | itda-web-collect:web-reader |
| 네이버 블로그 글·댓글·블로그 내 검색 | itda-web-collect:blog-reader |
| 시장조사 보고서·경쟁 분석 | itda-evidence-verify:market-scan |
| 주장·수치의 출처 검증 | itda-evidence-verify:ground-check |
| 로그인·JS 렌더가 필요한 사이트 | itda-web-collect:web-automation |
