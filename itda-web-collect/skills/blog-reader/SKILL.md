...
description: >
  ...
  [책임 경계] 본 스킬은 네이버 블로그(blog.naver.com) 전담 — 그 밖의 한국 웹페이지·EUC-KR·WAF 정적 페이지는 itda-web-collect:web-reader, 범용 키워드 검색은 itda-web-collect:web-search.
license: MIT
...
- **HTTP 페치는 같은 플러그인의 `web-reader` 스킬에 위임** — blog-reader 자체에는 HTTP 라이브러리
  ...
  itda-web-collect 를 설치하면 두 스킬이 함께 들어오므로 별도 준비가 필요 없습니다.
...
```
itda-web-collect/skills/web-reader   — fetch_html.py HTTP 페치 위임 (필수)
```
...
| 상황 | 대신 쓸 스킬 |
|---|---|
| 네이버 블로그가 아닌 웹페이지 본문 | itda-web-collect:web-reader |
| 여러 검색엔진 키워드 검색(출처 URL 수집) | itda-web-collect:web-search |
| 로그인·JavaScript 렌더·차단 사이트 | itda-web-collect:web-automation |
| 정보원 등급·플레이북 정찰 | itda-web-collect:web-scout |
