...
- GUIDE.md 빠른 시작을 **자기완결 예시**로 교체 (마스터 피드백) — 블로그 미지정 예시("네이버 블로그 글 가져와줘")는 되묻기를 유발하므로, 그대로 입력하면 결과까지 나오는 예시(discover 키워드 검색·블로그 ID 지정·포스트 URL 지정)로 전면 교체. 시나리오 예시에도 URL/ID 포함형 병기, "이 블로그" 화법은 대화 맥락용으로 명시.
