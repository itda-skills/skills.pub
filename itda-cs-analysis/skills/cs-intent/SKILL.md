...
2. **팬아웃** — `itda-cs-analysis:cs-batch-extractor` 를 청크별로 **병렬 명시 디스패치**한다(디스패치 프롬프트에 `task=cs-intent` + 이 파일의 closed-set 인텐트 체계·무상태 단건·고정 JSON 원칙 + 청크 파일 경로 + `outputs/` 출력 경로). 워커는 각 항목을 무상태로 분류해 스키마 호환 JSONL 을 `outputs/` 에 쓰고 경로·건수만 반환한다.
