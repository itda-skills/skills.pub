...
한국 정부 공공데이터 API를 활용한 Claude Code 스킬 모음입니다.
법령, 전자공시, 국가통계, 경제지표, 부동산 실거래가, 정부 지원사업, 나라장터 입찰공고를 조회합니다.

## 시작 전: API 키/OC 먼저 발급하세요

itda-gov의 대부분 스킬은 **사전 API 키(또는 OC) 발급이 필요**합니다. 먼저 아래 발급처에서 키를 받은 뒤 Claude Code 환경변수 또는 `.env`에 설정하세요.

| 환경변수 | 발급처 | 용도 |
|---------|-------|------|
| `LAW_API_OC` | https://open.law.go.kr/LSO/openApi | 법제처 API OC 등록 |
...
> **주의**
> - `LAW_API_OC`는 일반 API 키가 아니라 **법제처 Open API 포털에서 신청·승인받는 OC**입니다.
...
| 스킬 | 데이터 소스 | 핵심 데이터 |
|------|-----------|----------|
| [`law-korean`](skills/law-korean/SKILL.md) | 법제처 Open API | 법령 검색, 조문 원문 조회 |
...
4. 법적 근거
   → law-korean: search_law.py --query "{법령명}"
5. 보고서 종합
```
