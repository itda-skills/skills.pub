...
2. 표시된 Decoding 키와 "Claude 지침"(또는 개발자 `.env`)의 값이 동일한지 확인
...
| 오류 메시지 / 코드 | 원인 | 해결 방법 |
|-------------------|------|-----------|
| `KO_DATA_API_KEY가 설정되지 않았습니다` | 환경변수 미설정 | "Claude 지침"에 `KO_DATA_API_KEY=키` 추가(권장) — 지침의 키를 실행 시 환경변수로 주입. 개발자는 `.env`·셸 환경변수도 가능 |
...
| `resultCode=30` 등록되지 않은 서비스키 | Decoding 키 오류 | 마이페이지 일반 인증키를 재복사해 "Claude 지침"(또는 개발자 `.env`) 갱신 |
