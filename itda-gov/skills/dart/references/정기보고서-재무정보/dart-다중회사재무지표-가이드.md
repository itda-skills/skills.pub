# 다중회사 주요 재무지표 API 개발가이드

출처: https://opendart.fss.or.kr/guide/detail.do?apiGrpCd=DS003&apiId=2022002
추출일: 2026-04-28

## 기본 정보

| 메서드 | 요청URL | 인코딩 | 출력포멧 |
|--------|---------|--------|---------|
| GET | https://opendart.fss.or.kr/api/fnlttCmpnyIndx.json | UTF-8 | JSON |
| GET | https://opendart.fss.or.kr/api/fnlttCmpnyIndx.xml | UTF-8 | XML |

## 요청 인자

| 요청키 | 명칭 | 타입 | 필수여부 | 값설명 |
|--------|------|------|---------|--------|
| crtfc_key | API 인증키 | STRING(40) | Y | 발급받은 인증키(40자리) |
| corp_code | 고유번호 | STRING(8) | Y | 공시대상회사의 고유번호(8자리) |
| bsns_year | 사업연도 | STRING(4) | Y | 사업연도(4자리, 2023년 3분기 이후) |
| reprt_code | 보고서 코드 | STRING(5) | Y | 11013(1분기), 11012(반기), 11014(3분기), 11011(사업) |
| idx_cl_code | 지표분류코드 | STRING(7) | Y | M210000(수익성), M220000(안정성), M230000(성장성), M240000(활동성) |

## 응답 결과

| 응답키 | 명칭 | 출력설명 |
|--------|------|---------|
| status | 에러 및 정보 코드 | 메시지 설명 참조 |
| message | 에러 및 정보 메시지 | 메시지 설명 참조 |
| reprt_code | 보고서 코드 | 11013, 11012, 11014, 11011 |
| bsns_year | 사업 연도 | 2023 |
| corp_code | 고유번호 | 8자리 |
| stock_code | 종목 코드 | 6자리 |
| stlm_dt | 결산기준일 | YYYY-MM-DD |
| idx_cl_code | 지표분류코드 | M210000-M240000 |
| idx_nm | 지표명 | 영업이익률 등 |
| idx_val | 지표값 | 0.256 등 |

## 에러 코드

| 코드 | 메시지 |
|------|--------|
| 000 | 정상 |
| 010 | 등록되지 않은 키 |
| 011 | 사용할 수 없는 키 |
| 012 | 접근할 수 없는 IP |
| 013 | 조회된 데이터 없음 |
| 020 | 요청 제한 초과 |
| 021 | 조회 회사 개수 초과(최대 100건) |
| 100 | 필드의 부적절한 값 |
| 900 | 정의되지 않은 오류 |
