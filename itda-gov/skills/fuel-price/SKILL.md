...
name: fuel-price
description: >
  출장 유류비 기준가 브리핑 스킬입니다. 오피넷(한국석유공사) 주유소 평균 유가(전국·시도, 일/주/월)를 조회해
  기준가 요약 → km당 유류비 단가(연비·보정계수) → 직원 공지문까지 만듭니다.
  "이달 유류비 기준가 알려줘", "출장 유류비 단가 공지문 만들어줘", "인천 경유 월평균 유가", "이번 주 휘발유 추세",
  "기름값 얼마야"처럼 말하면 됩니다. API 키 없이 동작합니다.
  [책임 경계] 본 스킬은 평균 유가 기준가·단가·공지문 전담 — 주유소 위치·최저가 검색은 하지 않고, 환율은 itda-work:exchange-rate 가 맡습니다.
license: Apache-2.0
compatibility: Claude Code & Cowork
user-invocable: true
argument-hint: "[지역] [제품] [--term day|week|month] [--efficiency km/L] [--factor 1.2] [--notice|--detail|--json]"
...
metadata:
  ...
  tags: "fuel-price, opinet, gasoline, diesel, travel-expense, per-km-rate, korea, keyless"
---

# Fuel Price (출장 유류비 기준가 브리핑)

오피넷 **평균 판매가격**(부가세 포함, 원/리터)을 기준가로 삼아, 회사 규정의 자차 출장 유류비 단가를
계산하고 직원 공지문을 만듭니다. 관행 산식: **거리(km) × (기준유가 ÷ 연비 × 보정계수)**.
...
| 종료 시점 | `--end` | 최신 | 특정 시점 조회 — 일간 `YYYY-MM-DD`, 주간·월간 `YYYY-MM` (예: "7월 기준가" → `--term month --end 2026-07`, "8월 15일 시세" → `--term day --end 2026-08-15`). 오피넷 최신 시점 이후는 거부 |
| 연비 | `--efficiency` | (없음) | 회사 기준 연비 km/L. 주면 **km당 단가** 계산 |
| 보정계수 | `--factor` | `1.0` | 공인연비 대비 실주행 감가(예: 1.2). 회사 규정값 |
| 출력 | `--notice` / `--detail` / `--json` | 요약 1줄 | 공지문 / 기간별 표 / JSON |

- "기준가·정산·공지" 맥락이면 `--term month`(월평균 관행) + `--notice`.
- "추세·최근 흐름"이면 `--term week --periods 8 --detail`.
- "오늘·어제·현재 기름값"이면 `--term day`(최신 확정일 = 오피넷은 전일까지 확정 통계 — "오늘" 값은 새벽에 갱신된 **전일자**가 최신이다. 출력의 기간 라벨이 기준일이니 그대로 보여준다).
...
- 연비·보정계수를 사용자가 말하지 않았고 공지문을 요구하면, **회사 기준 연비를 한 번 묻는다**(모르면 연비 없이 실행 —
  공지문이 산식만 안내한다). 값을 지어내지 않는다.
...
python3 "$SKILL_DIR/scripts/fuel_price.py" [--region 인천] [--product 경유] [--term month] [--periods 3] \
    [--efficiency 12] [--factor 1.2] [--detail] [--notice] [--json]

# Windows
py -3 "$env:SKILL_DIR\scripts\fuel_price.py" --region 인천 --product 경유 --efficiency 12 --notice
```
...
- `python3 "$SKILL_DIR/scripts/fuel_price.py" --region 인천 --product 경유 --efficiency 12 --factor 1.2 --notice`
- `python3 "$SKILL_DIR/scripts/fuel_price.py" --term week --periods 8 --detail`
- `python3 "$SKILL_DIR/scripts/fuel_price.py" --term month --end 2026-07 --region 부산` — 지난 7월 부산 기준가
- `python3 "$SKILL_DIR/scripts/fuel_price.py" --term day --end 2026-08-15 --periods 5 --detail` — 8/11~8/15 일별
- `python3 "$SKILL_DIR/scripts/fuel_price.py" --term day --source api` — `OPINET_API_KEY` 가 있을 때만

### Step 4: Display

스크립트 stdout 을 **그대로** 보여준다(수치·출처 문구를 고쳐 쓰지 않는다). 공지문(`--notice`)은 카톡·메일에
붙일 수 있는 완성문이며, 사용자가 문체를 바꾸길 원하면 **숫자·기준일·출처 줄은 보존**한 채 다듬는다.
...
- 저작권: 산출물에 항상 **출처 "오피넷(한국석유공사)"** 를 명시한다(오피넷 저작권 정책). 내부 공지 용도는 무방,
...
| 공지문을 **정기적으로** 만들어 카톡·메일로 보내기 | Cowork 스케줄 + 메신저/이메일 스킬(본 스킬은 공지문 생성까지) |
| 국제유가(WTI·두바이) | 별도 — 본 스킬은 국내 주유소 판매가만 |

## 배경 — 왜 "주유소 찾기"가 아니라 "기준가 브리핑"인가

IGM 9기 사전설문(2026-08) 원문: **"외부 자료를 정기적으로 확인해 내부 공지 (유류비 시가 확인 후 공지)"** — 전국 현장
출장이 상시적인 소기업의 행정 총괄이 낸 과제. 정기 확인 → 내부 공지하는 "유류비 시가"는 개인 주유 팁이 아니라
**자차 출장 유류비 정산의 기준값**(오피넷 전국/월평균 관행)이다. 그래서 스킬의 산출은 가격 표가 아니라
**기준가 + 단가 + 공지문**이다(#1632).
...
### 후속 요청 이어가기
"경유도 같이", "연비 10 으로 다시" 같은 후속은 인자만 바꿔 재실행한다 — 응답이 결정론적이라 재조회 비용이 낮다.
