...
> **로컬 CLI 전용** (선택): `.env` 대신 `claude config set env.ECOS_API_KEY "발급받은_키"` 또는 셸 환경변수도 사용할 수 있습니다.
...
| 오류 | 원인 | 해결 방법 |
|------|------|-----------|
| `ECOS_API_KEY가 설정되지 않았습니다` | API 키 미설정 | 작업 폴더 루트에 `.env` 생성 → `ECOS_API_KEY=키` (로컬 CLI는 셸 환경변수도 가능) |
