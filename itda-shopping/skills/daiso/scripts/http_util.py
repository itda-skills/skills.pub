...
모든 외부 호출은 여기를 거친다(REQ-008). WebFetch 금지 — /auth/request JWT가
요청자 IP·User-Agent에 바인딩되므로 데이터센터 IP로 나가면 인증 호출이 실패한다(§2-B).

설계 원칙:
  - GET-json / POST-json / GET-text 3종.
  - 403/429 감지 시 errors.AntiBotBlockError(exit 4) — 봇 차단 우회 안 함(EXC-4).
  - 기타 HTTP/네트워크 오류는 errors.DaisoFetchError(exit 1).
  - 일시 오류(타임아웃·일시적 네트워크 오류·5xx) 1회 재시도.
  - throttle: 호출 간 최소 지연(module-level 마지막 호출 시각 기준). time.monotonic 사용.
  - 예외는 raise로 전파(weather-here의 tuple 반환과 다른 계약 — 본 스킬은 raise 기반).

L-5(수용): `_last_call_ts`는 모듈 전역이라 멀티스레드에서 안전하지 않다. 본 스킬은
단일스레드 CLI 스코프에서만 동작하므로 수용한다 — 라이브러리로 import해 동시 호출하면
throttle 간격이 정확하지 않을 수 있다(스레드별 격리 필요 시 별도 설계).
"""
...
from errors import AntiBotBlockError, DaisoFetchError
...
#: 응답 본문 최대 바이트 (L-4). 초과 시 DaisoFetchError로 거부 — 무제한 read 방지.
#: 다이소 응답은 검색 결과 수십 KB 수준이라 10MiB면 충분히 넉넉하다.
...
def apply_throttle(throttle: float) -> None:
    """공개 throttle 헬퍼 (M-2).

    http_util을 거치지 않고 urllib을 직접 호출하는 경로(auth.request_token의
    `/auth/request` GET 등)도 동일한 module-level throttle 시계를 공유하도록
    `_apply_throttle`를 노출한다. GET 직전 호출하면 직전 HTTP 호출과의 최소 간격을
    보장해 봇 차단을 예방한다.

    Args:
        throttle: 직전 호출과의 최소 간격(초). ≤0이면 대기 없음.
    """
...
def read_capped(resp: Any, url: str) -> bytes:
    """응답 본문을 MAX_RESPONSE_BYTES 상한까지 읽는다 (L-4).

    상한+1 바이트를 읽어 초과를 감지한다 — 초과 시 DaisoFetchError로 거부해
    무제한 read를 방지한다. urllib 직접 호출 경로(auth.request_token)도 공유한다.
    ...
    Raises:
        DaisoFetchError: 본문이 MAX_RESPONSE_BYTES를 초과할 때.
    """
    raw = resp.read(MAX_RESPONSE_BYTES + 1)
    if len(raw) > MAX_RESPONSE_BYTES:
        raise DaisoFetchError(
...
def _execute(req: urllib.request.Request, *, timeout: float) -> bytes:
    ...
    403/429 → AntiBotBlockError, 그 외 실패 → DaisoFetchError.
    본문은 MAX_RESPONSE_BYTES 상한까지만 읽는다(L-4).
    """
    ...
    for attempt in range(2):  # 최초 1회 + 재시도 1회
        try:
            ...
            # HTTPError는 status가 명확 — 차단/비차단 분기.
    ...
    raise DaisoFetchError(  # pragma: no cover
        f"요청에 실패했습니다: {req.full_url} ({last_exc})"
    )


def _parse_json(raw: bytes, url: str) -> Any:
    """응답 바이트를 JSON으로 파싱한다. 실패 시 DaisoFetchError."""
    try:
...
def http_get_json(
    ...
    *,
    ...
    Raises:
        ...
        DaisoFetchError: 기타 네트워크/파싱 오류 (exit 1).
    """
    _apply_throttle(throttle)
    full_url = _with_params(url, params)
    req = _build_request(full_url, method="GET", user_agent=user_agent)
...
def http_post_json(
    ...
    *,
    ...
    """JSON 본문으로 POST 요청 후 JSON을 반환한다.

    payload(dict 또는 list)를 `Content-Type: application/json` 본문으로 직렬화한다.
    인증 헤더 3종(Authorization/X-DM-UID/Cookie)은 extra_headers로 주입한다(AES 인증).

    Args:
        url: 요청 URL.
        payload: 요청 본문이 될 dict 또는 list.
        timeout: 타임아웃(초).
        user_agent: User-Agent 헤더.
        extra_headers: 추가 헤더(AES 인증 헤더 등).
        throttle: 직전 호출과의 최소 간격(초).
...
def http_get_text(
    ...
    *,
    ...
    """GET 요청 후 본문 텍스트(str)를 반환한다 (레거시 HTML 등).
    ...
    Raises:
        AntiBotBlockError: 403/429 (exit 4).
        DaisoFetchError: 기타 네트워크 오류 (exit 1).
    """
    _apply_throttle(throttle)
    full_url = _with_params(url, params)
    req = _build_request(full_url, method="GET", user_agent=user_agent)
