...
- **외부 스킬 산출물** — `itda-web-collect:web-search`·`itda-gov-collect:dart`·`itda-gov-collect:kosis`·`itda-day-organize:exchange-rate` 등이 수집한 자료. **`외부/` 폴더에 격리 적재**하고 `출처:`(어느 스킬·URL·API)·`수집일:`을 강제한다(사내 원본과 분리 — CLAUDE.md 적재 규칙 9).
