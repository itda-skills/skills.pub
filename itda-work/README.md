...
**itda-skills**는 Claude Cowork, Claude Code에 유용한 스킬을 추가하는 플러그인입니다.
...
| **itda-emoticon** | 카카오 이모티콘 제작 기획·가이드 | 📦 |
| **itda-stt** | 오디오 파일을 텍스트로 변환 | 📦 |
...
| **itda-font-guide** | 문서 유형에 맞는 무료 폰트를 추천하고 자동 설치 | 📦 |
| **itda-name-badge** | Excel/CSV 명단과 PPTX 템플릿으로 명찰 자동 생성 | 📦 |
| **itda-slide-ai** | Gemini 기반 AI 슬라이드 이미지/PPTX 생성 | 🌐 🔑 |
...
| itda-font-guide | — | — | 폰트 설치 시 네트워크 필요 |
| itda-name-badge | `python-pptx` `openpyxl` | — | — |
| itda-slide-ai | 의존성 문서 참조 | `GEMINI_API_KEY` | Gemini API, PPTX 생성 환경 |
| itda-exchange-rate | — | — | — |
| itda-etf-naver | — | — | — |
| itda-blog-seo | — | — | 네이버 데이터 조회 |
| itda-draft-post | — | — | — |
| itda-emoticon | — | — | — |
| itda-stt | 구현별 상이 | 선택 | 음성 입력 파일 |

**키 설정 방법**

```bash
# Claude Code 개인 설정 (권장)
...
Claude Code 채팅창에서 다음 메시지를 순서대로 입력하세요.
...
> **참고**: `claude` CLI 명령어로는 설치할 수 없습니다. 반드시 Claude Code 채팅 메시지로 입력해야 합니다.
...
### itda-font-guide — 폰트 추천 & 자동 설치

문서, 프레젠테이션, 디자인 작업에 어울리는 **무료 폰트**를 추천하고 자동으로 설치해 드립니다.

**주요 기능:**

- 문서 유형(공문서, PPT, 계약서, 마케팅 등)에 맞는 폰트 추천
- 한글 + 영문 폰트 조합 제안
- macOS / Windows / Linux 자동 설치
...
### itda-emoticon — 이모티콘 기획 가이드

카카오 이모티콘 제작을 위한 **컨셉, 문구, 세트 구성**을 도와줍니다.

**주요 기능:**

- 타깃별 캐릭터·감정 표현 아이디어 제안
- 세트 구성과 메시지 문구 브레인스토밍
- 제출 전 콘셉트 정리

### itda-slide-ai — AI 슬라이드 생성

텍스트나 문서를 바탕으로 **슬라이드 이미지와 PPTX**를 생성합니다.

**주요 기능:**

- 주제/문서 기반 발표용 슬라이드 생성
- 테마, 슬라이드 수, 언어 옵션 지원
- 이미지 결과물을 PPTX로 재구성

**필요 환경:** `GEMINI_API_KEY`

### itda-stt — 음성 텍스트 변환

회의 녹음이나 음성 파일을 읽어 **텍스트로 변환**합니다.
...
📦 표시 스킬(font-guide, imagekit 등)은 별도 설정 없이 바로 사용할 수 있습니다.
