...
| 스킬 | 설명 | 환경 |
|------|------|:----:|
| **itda-find-work** | 비개발자 직장인의 업무 자동화·문제해결 후보를 단계별 인터뷰로 발굴 (트랙 A 반복 업무 / 트랙 B 미지의 문제) | 📦 |
| **itda-plan-work** | 사용자 요청을 적합한 itda-* 스킬 조합으로 매핑해 실행 계획 한 장 메모 출력 (미러백 + ground-check 검증) | 📦 |

### 조사·검증

| 스킬 | 설명 | 환경 |
|------|------|:----:|
| **itda-ground-check** | 1차 소스 강제 조사 + 독립 검증 + 실사용 예시 확장 3단계로 환각·hedge 표현·자기검증 편향을 절차로 차단 (팩트체크·비교표·출처 정리) | 🌐 |
| **itda-investigate** | "왜 이렇게 느리지", "이 에러 원인이 뭐야"처럼 원인 모를 때 체계적 조사 (경쟁 가설 + 반증 실험) | 📦 |

### 환경·생활

| 스킬 | 설명 | 환경 |
|------|------|:----:|
| **itda-weather-here** | 현재 위치 또는 지정 지역의 날씨를 한국어로 빠르게 조회 (위치 미지정 시 IP 지오로케이션 자동 탐지, Open-Meteo 무키) | 🌐 |
