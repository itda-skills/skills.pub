...
- 💡 **계획 단계가 필요하면 plan-work로 이어가세요**. 자동화 실행 직전, 어떤 도구를 어떤 순서로 쓸지 단계별 실행 계획을 한 장으로 정리해주는 자매 스킬입니다. 메모를 만든 직후 같은 대화에서라면 첨부 없이 "이어서 계획 세워줘"라고만 하면 되고, 새 세션이라면 find-work 메모를 첨부하고 "이 메모 기반으로 계획 세워줘"라고 말하면 됩니다.
