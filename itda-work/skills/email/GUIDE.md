...
네이버·Gmail·다음·카카오 메일은 물론, 회사 메일 서버까지 한 줄로 보내고 읽는 가장 간단한 방법입니다.
...
이 한 줄이면 어느 메일 서비스(네이버·Gmail·다음/카카오)인지 자동 판별하고, 등록된 계정에서 발송 또는 수신 결과를 정리해 보여줍니다.

## 처음 설정하기

메일 서비스는 보안 때문에 계정 비밀번호를 그대로 쓰지 못합니다. **2단계 인증을 켠 뒤 "앱 비밀번호"를 따로 발급**받아, 그 값을 프로젝트 `CLAUDE.md`(권장) 또는 작업 폴더의 `.env` 파일에 적어두면 됩니다. `CLAUDE.md`에 적어두면 Claude가 자동으로 읽습니다.
...
4. `NAVER_EMAIL` / `NAVER_APP_PASSWORD`를 `CLAUDE.md` 또는 `.env`에 넣습니다.
...
4. `ICLOUD_EMAIL` / `ICLOUD_APP_PASSWORD`를 `CLAUDE.md` 또는 `.env`에 넣습니다. `@icloud.com`·`@me.com`·`@mac.com` 모두 동일하게 동작합니다.

> 기존에 `--provider custom`으로 iCloud 서버를 직접 설정하던 분은 `ICLOUD_EMAIL`·`ICLOUD_APP_PASSWORD` 두 변수만으로 대체할 수 있습니다.

### 회사 메일 등 직접 설정(커스텀)

사내 메일 서버를 쓴다면 보내기용 `SMTP_HOST` `SMTP_PORT` `SMTP_USER` `SMTP_PASSWORD`를, 받기까지 하려면 `IMAP_HOST` `IMAP_PORT`를 추가로 넣습니다.

> `GMAIL_ADDRESS` / `GMAIL_APP_PASSWORD`를 쓰던 분은 이름만 `GOOGLE_EMAIL` / `GOOGLE_APP_PASSWORD`로 바꾸면 됩니다. "네이버/Gmail 메일 보내줘"의 `gmail` 표현은 계속 그대로 통합니다. 증분 조회 커서도 다음 실행 때 자동으로 옮겨집니다.
...
- **본문·수신자·첨부 분리 표시** — HTML로 작성된 메일도 본문이 깨지지 않고 그대로 보이며, 첨부 파일은 파일명·종류·크기가 별도 목록으로 표시됩니다. 받는 사람(`to`)·참조(`cc`)도 이름과 주소로 함께 표시되어 누구에게 보낸 메일인지 한눈에 확인됩니다.
- **본문 형식 표기** — HTML로 작성된 메일이면 `body_format: html`, 일반 텍스트는 `body_format: text`로 표시됩니다. 텍스트 본문을 우선 보고 싶을 땐 "텍스트 본문 우선으로 보여줘"라고 말하면 `--prefer-text` 옵션이 적용됩니다.
...
- **계정 정보는 `CLAUDE.md`에 (권장)** — 프로젝트의 `CLAUDE.md`에 이메일·앱 비밀번호를 적어두면 자동으로 읽힙니다. `.env`보다 우선 권장됩니다.
...
- **`GMAIL_*` 환경변수 정리** — 예전 이름(`GMAIL_ADDRESS` / `GMAIL_APP_PASSWORD`)을 쓰던 분은 `GOOGLE_EMAIL` / `GOOGLE_APP_PASSWORD`로 바꿔주세요. "gmail로 보내줘"라는 자연어 표현은 그대로 통합니다.
