...
**Claude에게 말로 부탁하면 됩니다** — `/hwpx-reader 이 HWP 파일 읽어줘`, `/hwpx-reader 한글 문서 마크다운으로 바꿔줘`처럼요. 자연어로 말해도 그대로 동작합니다 — `/hwpx-reader` 접두는 생략 가능합니다.
...
```
/hwpx-reader 이 HWP 파일 읽어줘
/hwpx-reader 한글 문서를 마크다운으로 변환해줘
/hwpx-reader 서식 그대로 HTML로 변환해줘
```

스킬이 자동으로 변환 도구를 찾아 실행합니다. 이미지가 있으면 추출은 하지만 이미지 설명(캡션)은 기본적으로 달지 않습니다 — 필요하면 `/hwpx-reader 이미지 설명도 달아줘`를 덧붙이세요.
...
```
/hwpx-reader 이 보고서를 마크다운으로 변환해줘
```
...
```
/hwpx-reader HWP를 HTML로 변환해줘. 서식 그대로
```
...
```
/hwpx-reader 이 HWP가 MD로 이상하게 변환됐어. HTML로 우회해줘
```
...
```
/hwpx-reader 이 보도자료 본문만 마크다운으로 뽑아줘
/hwpx-reader 이 보고서 변환할 때 이미지 설명도 달아줘
```
...
```
/hwpx-reader 이 문서 원본 그대로 표 형식으로 변환해줘
```
