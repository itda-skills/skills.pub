...
**9. SPA 어댑터 인프라** (v2.7.0)
WebSquare5/Nexacro 기반 SPA 사이트를 위한 어댑터 인프라. deep-link 차단·가상 그리드·`.do` POST JSON 대응 프레임워크.
사용자가 `scripts/spa_adapters/`에 커스텀 어댑터를 작성하고 `manifest.json`에 등록해 `--adapter` 옵션으로 실행할 수 있다.
도메인 특화 어댑터(감지·캡처·매니페스트 기반 사용자 정의)는 별도 hyve 프로젝트에서 관리된다.
