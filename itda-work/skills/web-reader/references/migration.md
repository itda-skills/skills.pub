...
| 네이버 부동산 단지/매물 | hyve MCP `web_browse` (`observe{network}`로 단지/매물 XHR API 캡처) |
...
| `fetch_dynamic.py --adapter naver_land --adapter-page complexes` | hyve MCP `web_browse` (`observe{network}`로 단지/매물 XHR 캡처) |
| `fetch_dynamic.py --adapter naver_land --adapter-page complex_detail` | hyve MCP `web_browse` (단지 + 매물 XHR API 캡처) |
| `extract_content.py --from-capture <jsonl> --adapter naver_land` | hyve MCP `web_browse` (리뷰/단지 XHR 캡처 후 파싱) |
...
After (v3.0.0+):
```
hyve의 web_browse로 104917 단지 페이지 열고 observe network로 매물 API 원본 캡처해서 JSON으로 받아줘
```
