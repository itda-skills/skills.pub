...
v3.0.0 폐기 플래그 (호출 시 exit 4 + hyve MCP 안내):
  --dynamic-only            → hyve MCP web_browse (SPEC-WEB-MCP-002) 사용
...
| v2.x 호출 | v3.0.0 대체 |
|-----------|-------------|
| `fetch_dynamic.py --url URL` | hyve MCP `web_browse` (SPEC-WEB-MCP-002) |
| `--adapter naver_land` (단지/매물) | hyve MCP `naverplace.complex` / `naverplace.search` / `naverplace.reviews` |
| `--from-capture <jsonl>` | hyve MCP `web_browse` capture 모드 |
| `extract_content.py --dynamic-only` | hyve MCP `web_browse` |
