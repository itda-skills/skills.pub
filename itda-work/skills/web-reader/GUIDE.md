...
다음 표는 web-reader 단독으로 처리 가능한 사이트를 카테고리별로 실측한 결과입니다. **대부분 정적 fetch(`fetch_html.py`)만으로 충분**하며 — Next.js SSR이 보편화되면서 SPA처럼 보여도 본문은 정적으로 같이 내려오는 경우가 많기 때문입니다. lightpanda는 client-side fetch에만 의존하는 진성 CSR SPA에서만 필요합니다.

### 정적 fetch로 충분한 사이트 (lightpanda 불필요)

| 카테고리 | 사이트·URL | 실측 추출량 | 활용 |
|---|---|---|---|
| 한국 미디어 | `news.naver.com/section/{100,101,105}` (IT/경제) | 7,174 words | 매일 아침 헤드라인 클립핑 |
...
```bash
# 가장 단순한 형태 — 정적 fetch + markdown 추출 한 줄
python3 scripts/fetch_html.py --url "https://news.naver.com/section/105" | \
  python3 scripts/extract_content.py --format markdown --url "https://news.naver.com/section/105"
```

### Lightpanda가 필요한 사이트 (실측 정적 fetch 실패 → 동적 OK)
...
python3 scripts/extract_content.py \
  ...
  --dynamic-only --format markdown

# 또는 lightpanda 자체 markdown 덤프 (정제 우회, 한국 미디어·일부 SPA에 더 빠름)
python3 scripts/extract_content.py \
  --url "https://disquiet.io/" \
  --dynamic-only --lp-markdown
```

### 의사결정 휴리스틱

1. **먼저 정적 fetch 시도** — 1초 안에 끝나고 lightpanda 부팅·메모리 비용 없음
2. `extract_content.py` 결과가 **`words=0` 또는 본문이 비어있으면** → `--dynamic-only` 재시도
3. `--dynamic-only`도 비어있거나 **exit 4(bot challenge)** 면 → 해당 사이트는 web-reader 단독 범위 밖 (hyve MCP 영역)
...
- **CSR SPA 진성 케이스**: Wanted(CRA 추정), Disquiet(client-side fetch 위주) — 이런 곳에서만 lightpanda 진정한 효용 발휘
- **alio.go.kr 같은 정부 사이트는 `/main.es` 같은 옛 URL이 404**일 수 있으니 루트(`/`)로 먼저 시도할 것
