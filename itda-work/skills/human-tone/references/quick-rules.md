...
| A-2 | "~를 통해/통하여" 문서 내 3회+ 남발 | S2 | 남발분만 "~로", "~해서", "~함으로써"로 분산. 1~2회는 보존(원어민이 2배 더 씀 — empirical-validation) |
...
| A-16 | "그/그녀/그것/그들" 단락 ≥3회 영어 대명사 직역 — **번역 맥락 전용**(자생 한국어 산문 미발동) | S1 | 50%+ 영형(생략) 또는 호칭·명사구로 (김도훈 2009). 자생 산문의 `그는/그의` 는 보존 |
...
| ID | 패턴 | 심각도 | 처방 |
|---|---|---|---|
| I-1 | "~인 것이다/~한 것이다" 결말 **연속 3회+** | S2 | 남발분 일부만 평서형으로. 단발은 보존(사람이 2배 더 씀 — empirical-validation) |
...
5. **잔존 S1 패턴 0건**: D-1~D-7, A-7, A-8, C-5, C-10, C-11, H-1, J-2 핵심 S1이 남아있지 않은가 (A-16 은 번역 맥락에서만, I-1·A-2 는 S2 조건부 — 2026-09 #1619)
