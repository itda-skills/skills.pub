...
```
/work-find Cowork로 뭘 해볼 수 있을지 같이 찾아줘
```

```
/work-find 내가 반복적으로 하는 일 중에 자동화할 만한 거 찾아줘
```

```
/work-find 아직 손 못 댄 일 중에 풀어볼 만한 거 같이 골라줘
```
...
```
/work-find 매주 월요일에 거래처 메일 정리하는 게 너무 오래 걸려. 자동화 가능한지 봐줘
```
...
```
/work-find 신사업 후보 기업을 좀 발굴해보고 싶은데 어떻게 시작해야 할지 모르겠어
```
...
```
/work-find 지난번에 만든 work-find/2026-05-12_0930_거래처메일분류.md 이어서 보강해줘
```
...
연결 폴더 `work-find/` 아래에 `YYYY-MM-DD_HHmm_<키워드>.md` 형식으로 저장됩니다.
...
- 💡 **계획 단계가 필요하면 work-plan로 이어가세요**. 자동화 실행 직전, 어떤 도구를 어떤 순서로 쓸지 단계별 실행 계획을 한 장으로 정리해주는 자매 스킬입니다. 메모를 만든 직후 같은 대화에서라면 첨부 없이 "이어서 계획 세워줘"라고만 하면 되고, 새 세션이라면 work-find 메모를 첨부하고 "이 메모 기반으로 계획 세워줘"라고 말하면 됩니다.
