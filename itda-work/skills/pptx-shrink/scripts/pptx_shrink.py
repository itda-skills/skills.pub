...
def analyze(infile: str, min_bytes: int, quality: int, top: int = 10) -> dict:
    ...
    return {
        ...
        "est_saved_bytes": est_saved,
        "est_saved_mb": _mb(est_saved),
        "est_after_mb": _mb(max(total - est_saved, 0)),
        "top": images[:top],
        "params": {"min_bytes": min_bytes, "quality": quality},
    }
...
def print_report(rep: dict) -> None:
    ...
    print(f"예상: {rep['file_mb']} MB → {rep['est_after_mb']} MB (절감 {rep['est_saved_mb']} MB)")
    if rep["top"]:
        print("상위 이미지:")
        for r in rep["top"]:
            dim = f"{r['width']}×{r['height']}" if r["width"] else "-"
            est = f" → {_mb(r['est_bytes'])} MB" if r.get("est_bytes") else ""
            print(f"  {r['name']:18s} {_mb(r['bytes']):7.2f} MB {dim:>10s}  {r['action']}{est}")


# ── shrink ────────────────────────────────────────────────────────────────────

def shrink(infile: str, outfile: str, min_bytes: int, quality: int) -> dict:
    ...
    result = {"converted": converted, "kept": kept, "saved_bytes": saved, "skipped": skipped}
    if not converted:
        return result
...
def cmd_shrink(a: argparse.Namespace) -> int:
    ...
    os.close(fd)
    try:
        res = shrink(infile, tmp, a.min_bytes, a.quality)
    ...
    payload = {
        ...
        "converted": res["converted"], "kept": res["kept"],
        "image_saved_mb": _mb(res["saved_bytes"]), "skipped": res["skipped"],
        "verify": vres,
        "params": {"min_bytes": a.min_bytes, "quality": a.quality},
    }

    def _p(p: dict) -> None:
        print(f"변환: {p['converted']}장, 유지: {p['kept']}장 (이미지 절감 {p['image_saved_mb']} MB)")
