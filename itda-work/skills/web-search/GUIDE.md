...
발급받은 키는 **Claude 지침(개인 지침)**에 추가합니다 — Claude Desktop → **설정 → 일반 →
"Claude 지침"** 에 사용할 엔진의 키만 붙여넣고 저장하세요. 한 번 넣으면 이후 모든
Cowork 대화에서 자동으로 쓰입니다.
...
> 키 외에 `.env` 같은 파일을 따로 만들 필요는 없습니다 — 위 "Claude 지침"에 넣은 키만으로 동작합니다.
