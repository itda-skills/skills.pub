...
발급받은 키는 **Claude Desktop 지침**에 추가합니다 — Claude Desktop → **설정 → 일반 →
...
> `.env` 같은 파일을 직접 만들 필요가 없습니다. 개발자라면 작업 폴더 루트의 `.env`에 같은
> 줄을 넣어도 됩니다.
...
| 네이버 | 하루 25,000번 무료 | 무료 | developers.naver.com |
...
국내 결과가 필요하면 네이버로, 뉴스만 보고 싶으면 그렇게 말하세요.
