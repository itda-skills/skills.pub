...
### 키 등록 — Claude Desktop 지침에 등록 (권장, 비개발자 포함 모든 사용자)

Claude Desktop → **설정 → 일반 → "Claude 지침"** 에 사용할 엔진의 키만 붙여넣고 저장하면
이후 모든 대화에서 자동 사용됩니다(파일 작성 불필요).
...
> **키 주입 (Claude 실행 규칙):** API 키가 환경(`os.environ`·`.env`)에 없고 사용자
> 지침("Claude 지침"·`CLAUDE.md`)에 위 키가 있으면, 그 값을 읽어 스크립트 실행 시
> 환경변수로 전달한다 — 예: `TAVILY_API_KEY=<키> NAVER_SEARCH_CLIENT_ID=<키>
> NAVER_SEARCH_CLIENT_SECRET=<키> python3 scripts/web_search.py "검색어"`. 설정된
> 엔진 키만 주입하면 되고, 지침에도 없으면 GUIDE의 발급 안내를 제시한다.
