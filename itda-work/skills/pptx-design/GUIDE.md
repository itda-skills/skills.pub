...
> **디자인을 눈으로 확인하려면** [PPT 디자인 갤러리 (skills.itda.work/pptx-design)](https://skills.itda.work/pptx-design/)에서 20종 실제 덱의 미리보기·PDF·PPTX를 볼 수 있습니다. 이 문서는 글이라 색·레이아웃이 그대로 보이지 않습니다.
...
```
"/pptx-design 이 내용으로 발표자료 만들어줘"                          ← 주제에 맞는 디자인 자동 선택
"/pptx-design MBB 같은 컨설팅펌 스타일로 삼성전자 주가 보고서 만들어줘"  ← 스타일 프리셋 적용 (§2.2)
"/pptx-design 다크 트레이딩 톤으로 시장 모니터링 덱 10장"              ← 프리셋 + 장수 지정
"/pptx-design 우리 회사 브랜드 컬러(#1A73E8)로 IR 덱 5장"             ← 자사 컬러 즉석 적용
"/pptx-design 이 DESIGN.md 적용해서 슬라이드 만들어줘 (URL/파일)"      ← 웹 DESIGN.md 토큰 적용
"/pptx-design 우리 브랜드 DESIGN.md 파일로 만들어서 저장해줘"          ← 조직 디자인 박제 (§4.3)
```
...
| **디자인 지정** | 선택 | 프리셋 이름·톤 한마디·DESIGN.md(파일/URL)·자사 컬러. 아무것도 안 주면 주제에 맞는 톤 후보 몇 개를 보여주고 고르게 합니다("알아서 골라줘"도 가능). |
...
1. **그냥 맡기기 (이제 후보를 보여주고 고르게 합니다)** — 디자인을 전혀 언급하지 않으면, 주제·톤에 맞는 **톤 후보 몇 개(예: 따뜻한 에디토리얼 · 다크 트레이딩)를 추려 보여주고** 한 번 고르게 합니다. 고르기가 번거로우면 **"주제에 맞게 알아서 골라줘"** 를 선택하면 되고, 그러면 예전처럼 가장 잘 맞는 톤을 알아서 적용하고 선택 근거를 한 줄로 알려 드립니다(자동화로 돌리는 등 물어볼 수 없는 상황에서도 알아서 적용). **짧게 대충 말해도 결과 기준은 같습니다** — 내용이 적으면 장수를 줄일지언정 슬라이드를 헐겁게 만들지 않고, 콘텐츠를 가운데에 몰아넣거나 구분 부호를 남발하는 "AI 티 나는" 스타일은 자동 점검으로 걸러냅니다.
...
`../design-core/library/` 에 들어 있는 ready-to-use 디자인입니다. 이름이나 톤으로 가리키면 됩니다.
...
```
"/pptx-design MBB 같은 티어1 컨설팅펌 스타일로 삼성전자 주가 보고서 프리젠테이션을 만들어줘"
...
```
"/pptx-design 우리 브랜드 컬러는 #1A73E8(파랑)·#202124(잉크)·#FBBC04(옐로 액센트)이고,
...
```
"/pptx-design 우리 회사 브랜드로 DESIGN.md 만들어서 저장해줘.
...
```
"/pptx-design 우리 DESIGN.md 로 3분기 실적 보고 덱 만들어줘"
```
...
→ "/pptx-design 이 DESIGN.md 로 만들어줘"라고 하면 토큰을 해석해 반영합니다. 더 풍부한 출발점이 필요하면 `../design-core/library/` 의 프리셋 파일을 복사해 색만 바꾸는 것을 권합니다.
...
- 스타일 프리셋 6종(팔레트+슬라이드 문법): `../design-core/library/` (선택 표: 그 안의 README)
