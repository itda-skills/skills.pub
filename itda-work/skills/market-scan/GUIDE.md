...
- 인접 스킬: `ground-check`(엄격 검증) · `web-reader`(본문 추출) · `investigate`(원인 규명) · `data-ask`(내 데이터 분석) · `itda-gov:*`(공공데이터)
