...
b. web_browse session.new {mode:"shared", profile_id:"default"}
...
| 모달이 클릭을 가려 `interact{click}` 이 30s 타임아웃(스택 LUXDialog 등) | `session.new {auto_dismiss_dialogs:true}` 로 서버사이드 dismiss 워처 가동(#450). 직접 DOM click 이라 마우스 클릭이 못 닫는 스택 모달도 닫힘. 비파괴(닫기/X 만) |
