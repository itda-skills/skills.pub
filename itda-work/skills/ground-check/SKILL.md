...
정본은 **에이전트 명시 디스패치**다 — Task 2 는 `itda-work:ground-verifier` 로의 명시 디스패치를 정본으로 하고(§Task 2 환경 분기), 순수 자연어 발화만으로는 독립 서브에이전트가 자동 위임되지 않으므로(능력 지도 실측: 자동 위임 미발동) 자연어 경로는 **degraded 폴백**이다. 아래는 각 절차의 Claude Code SDK(`@anthropic-ai/sdk`, Python Agent SDK) 등가 표기 매핑이다.

| Cowork 절차 (명시 디스패치 정본) | Claude Code SDK 등가 표기 |
|---------------------|----------------------|
| Task 2 검증 지침 블록 → ground-verifier 디스패치 | `Agent(subagent_type: "itda-work:ground-verifier", prompt: "<검증 지침 블록 + 입력 계약>")` — tools 상속(에이전트 정의가 소유). 에이전트 부재 환경은 `Agent(subagent_type: "general-purpose", prompt: "<검증 지침 블록>", allowed_tools: ["WebSearch", "WebFetch", "Skill", "Read"])` 또는 새 세션 handoff 로 degraded 폴백 |
