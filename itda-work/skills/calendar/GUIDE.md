...
발급받은 값을 `.env` 파일(또는 환경변수)에 넣습니다. 로그인 아이디는 **전체 이메일**을 씁니다.
...
표준 CalDAV 서버는 서버 주소(`CALDAV_URL`)로 직접 연결합니다. 대부분 2단계 인증을 켜면 **앱 비밀번호**가 필요합니다(일반 비밀번호는 거부될 수 있음). 아래 값을 `.env`에 넣습니다.
...
| 증상 | 원인 / 해결 |
|------|-------------|
| "자격증명이 없다"는 안내 (`credentials_missing`) | `.env`에 해당 캘린더의 이메일·앱 비밀번호가 비어 있음. 위 "처음 설정하기" 확인 |
