...
**Claude에게 말로 부탁하면 됩니다** — `/kacem-tender 이 공고 사업비 정리해줘`, `/kacem-tender 발주처랑 공급가액 뽑아줘`처럼요.
...
| 하고 싶은 것 | 이렇게 말하세요 |
|---|---|
| 공고 파일 전체 정리 | `/kacem-tender 이 hwpx에서 사업개요랑 사업비 표로 뽑아줘` |
| 특정 항목만 추출 | `/kacem-tender 이 공고에서 발주처랑 공급가액만 뽑아줘` |
| 여러 파일 한 번에 | `/kacem-tender 이 폴더 안에 있는 공고 파일 전부 정리해줘` |
| CSV도 함께 저장 | `/kacem-tender 표 정리하고 CSV 파일도 같이 만들어줘` |
| 요약만 확인 | `/kacem-tender 공고 요약을 표로 보여줘` |
...
`kacem-tender fetch 서브커맨드` 스킬로 군인공제회 게시판에서 공고 파일을 먼저 받아두면, 이 스킬이 그 파일을 바로 읽어 처리합니다. `/kacem-tender 공고 받아서 사업비 정리까지 해줘`처럼 말하면 두 스킬이 이어서 동작합니다.
