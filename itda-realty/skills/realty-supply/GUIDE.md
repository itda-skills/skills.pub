...
청약 경쟁률·분양 데이터를 함께 보려면 공공데이터포털 키도 필요합니다. `realty-deals` 스킬을 이미 쓰고 있다면 같은 키를 그대로 씁니다. 이 키도 같은 자리(**설정 → 일반 → "Claude 지침"**)에 한 줄 추가하면 됩니다.
...
| 증상 | 원인 / 해결 |
|------|-------------|
| "KOSIS 키가 없다"는 안내 (`config`) | **Claude 지침**(또는 `.env`)에 `KOSIS_API_KEY`가 없거나 비어 있음. 위 "처음 설정하기" 확인 |
| "공공데이터 키가 없다"는 안내 (`config`) | 청약 데이터 요청 시 `KO_DATA_API_KEY`도 필요. **Claude 지침**(또는 `.env`)에 추가 |
