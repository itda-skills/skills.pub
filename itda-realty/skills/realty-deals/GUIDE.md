...
3. 발급받은 API 키를 **Claude Desktop 지침**에 추가합니다 — Claude Desktop → **설정 → 일반 → "Claude 지침"** 에 아래 한 줄을 붙여넣고 저장하세요.
...
| 증상 | 원인 / 해결 |
|------|-------------|
| "API 키가 없다"는 안내 (`config`) | **Claude 지침**(또는 `.env`)에 `KO_DATA_API_KEY`가 없거나 비어 있음. 위 "처음 설정하기" 확인 |
