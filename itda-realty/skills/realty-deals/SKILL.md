...
> **저장소 체크아웃에서 직접 실행 시(개발자)**: 공용 모듈(`deals_collector`·`data_go_client`·`lawd_codes`·`env_loader`)이 `itda-realty/shared/`·루트 `shared/` 에 있어 `PYTHONPATH=itda-realty/shared:shared` 를 붙여야 합니다. 배포본은 publish 주입으로 불필요.
