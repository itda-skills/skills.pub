...
SKILL_DIR="${CLAUDE_PLUGIN_ROOT:+$CLAUDE_PLUGIN_ROOT/skills/welfare-portal}"
[ -n "$SKILL_DIR" ] || SKILL_DIR=$(find /sessions/*/mnt/.remote-plugins -type d -path '*/skills/welfare-portal' 2>/dev/null | head -1)
...
```powershell
$env:SKILL_DIR = "$env:CLAUDE_PLUGIN_ROOT\skills\welfare-portal"  # 미설정이면 SKILL.md 위치 절대경로 사용
```
