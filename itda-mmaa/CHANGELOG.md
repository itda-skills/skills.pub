...
군인공제회(MMAA) 업무 자동화 스킬팩 — KACEM 게시판 모니터링 + ZIP 첨부 자동 압축해제 + 사업개요·사업비 추출 + 복지포털 스냅샷 Q&A.

## 2026-07-27 (이슈 #1306) — v0.15.0

### Changed

- **kacem-tender-fetch + kacem-tender-extract → kacem-tender 통합 (#1306, 마스터 결정)** — 한 pain('공고 수집→사업비 정리')의 단계 분할이던 2스킬을 단일 스킬(v2.0.0)로 통합. `fetch` 서브커맨드 편입 + `fetch --extract` 원스톱(core_document 상대경로 핸드오프 마찰 해소, 라이브 실측). 트리거 중복·문서/테스트/CI 2벌 유지비 제거.

## 2026-07-27 (이슈 #1305) — v0.14.0

### Removed

- **webmail 스킬 제거 (#1305, 마스터 결정)** — 실사용 검증이 안 된 스킬이라 배포 중단. SPEC-KACEM-WEBMAIL-001 은 재검증·재도입 시 참조용으로 존치. CI 매트릭스·README·keywords 동반 정리.
