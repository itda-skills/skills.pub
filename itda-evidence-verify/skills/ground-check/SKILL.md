...
- WebFetch 실패 시 **`itda-web-collect:web-reader` 스킬로 fallback** 한다 (§ Fallback 체인 참조 — 별도 팩, 미설치면 폴백 없이 실패 기록)
...
5. WebFetch 실패 시 `itda-web-collect:web-reader` 스킬로 fallback 할 것(미설치면 폴백 없이 실패로 기록)
6. 결과를 다음 표로 출력:
...
## Fallback 체인 — WebFetch → itda-web-collect:web-reader
...
1. WebFetch 시도
2. 실패 감지 → `itda-web-collect` 설치 여부 확인(스킬 목록에 web-reader 부재면 **폴백 없이** "itda-web-collect 미설치 — WebFetch 폴백 불가" 를 셀에 기록하고 4번으로) → 본 세션이 다음을 발화: **"web-reader 스킬로 본문을 가져와줘 (URL: …)"**
3. web-reader 결과로 본문 확보 → Task 1-A 셀 기록
4. web-reader도 실패 → 해당 URL 인용 금지, 다른 1차 소스 재탐색
5. 1차 소스 후보 모두 실패 → 셀 "미확인" 강등

> Cowork 환경에서는 `Skill("itda-web-collect:web-reader")` 코드 호출 대신 위 자연어 발화로 web-reader 활성화를 유도한다. **web-reader 는 다른 팩(`itda-web-collect`)이다** — 스킬이 없으면(로드 실패·미인식) 폴백을 시도하지 말고 "itda-web-collect 미설치로 WebFetch 폴백 불가" 를 결과에 그대로 적고 그 URL 은 실패로 센다.
...
정본은 **에이전트 명시 디스패치**다 — Task 2 는 `itda-evidence-verify:ground-verifier` 로의 명시 디스패치를 정본으로 하고(§Task 2 환경 분기), 순수 자연어 발화만으로는 독립 서브에이전트가 자동 위임되지 않으므로(능력 지도 실측: 자동 위임 미발동) 자연어 경로는 **degraded 폴백**이다. 아래는 각 절차의 Claude Code SDK(`@anthropic-ai/sdk`, Python Agent SDK) 등가 표기 매핑이다.

| Cowork 절차 (명시 디스패치 정본) | Claude Code SDK 등가 표기 |
|---------------------|----------------------|
| Task 2 검증 지침 블록 → ground-verifier 디스패치 | `Agent(subagent_type: "itda-evidence-verify:ground-verifier", prompt: "<검증 지침 블록 + 입력 계약>")` — tools 상속(에이전트 정의가 소유). 에이전트 부재 환경은 `Agent(subagent_type: "general-purpose", prompt: "<검증 지침 블록>", allowed_tools: ["WebSearch", "WebFetch", "Skill", "Read"])` 또는 새 세션 handoff 로 degraded 폴백 |
| "web-reader 스킬로 가져와줘" 발화 | `Skill("itda-web-collect:web-reader")` 도구 호출 또는 직접 스크립트 실행(미설치면 위 규칙대로 실패 보고) |
...
- 인접 스킬: `itda-web-collect:web-reader` — WebFetch fallback 대상(**다른 팩** — `itda-web-collect` 설치가 전제. 미설치면 폴백 없이 WebFetch 실패를 그대로 보고한다)
