...
- 단순 서술 정리·보고서화는 `itda-content-create:draft-post`(작성)가 담당 — 본 스킬은 검수
...
**`itda-evidence-verify:meeting-reliability-worker`** 가 있으면, Lead 는 녹취를 본 컨텍스트로
...
- 서술형 회의록 작성·보고서화: `itda-content-create:draft-post`. 본 스킬은 검수(변환+verify).
