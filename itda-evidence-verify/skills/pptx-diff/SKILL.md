...
pptx 를 **만드는** 일은 `itda-content-create:pptx-design` 이, **읽어서 내용을 뽑는** 일은 문서 스킬이 한다.
...
`itda-content-create:pptx-design` 등으로 덱을 다시 만든 뒤, 의도한 것만 바뀌었는지 본다.
...
- `itda-content-create:pptx-design` — 덱 생성. 이 스킬은 그 산출물의 회귀 확인에 쓰인다(시나리오 ①).
- `itda-web-collect:web-automation` — 같은 플러그인의 hyve MCP 소비 스킬. 웹 자동화 레시피 정본.
