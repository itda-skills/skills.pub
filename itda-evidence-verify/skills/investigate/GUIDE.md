...
**Claude에게 말로 부탁하면 됩니다** — `/investigate 이 에러 원인이 뭐야?`, `/investigate 왜 이렇게 느리지 분석해줘`처럼요.
...
```
/investigate 이 에러 원인 파헤쳐줘
/investigate 왜 이렇게 느리지 조사해줘
/investigate 이 설계가 맞는지 확인해줘
/investigate 이 메트릭 급등이 왜 일어난 거지?
```
...
```
/investigate 이 ImportError 원인 찾아줘
```
...
```
/investigate API가 배포 후 갑자기 10배 느려졌어. 원인 분석해줘
```
...
```
/investigate 이 모놀리스를 마이크로서비스로 분리하는 게 맞을까?
```
...
```
/investigate 대시보드 API 요청 수가 갑자기 10배 늘었는데 뭘 의미하지?
```
...
```
/investigate 이 에러 원인 분석해서 reports/bug-analysis.md로 저장해줘
```
