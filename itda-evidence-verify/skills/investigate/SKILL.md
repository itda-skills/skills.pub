...
**서브에이전트 팬아웃 (선택)** — 완전 모드에서 독립 가설·증거 축이 여러 개면, 환경에 `itda-evidence-verify:deep-researcher` 서브에이전트가 있을 때 축별로 병렬 위임할 수 있다(가설 + 검증할 증거 + 반증 요구를 프롬프트로). 격리 컨텍스트가 로그·페이지 원문을 흡수하고 구조화 요약만 반환하므로 본 조사 컨텍스트가 오염되지 않는다. 반환된 증거도 아래 방법론의 반증·신뢰도 보정을 동일하게 거친다. 서브에이전트가 없으면 본 컨텍스트에서 순차 진행한다.
