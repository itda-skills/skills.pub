...
description: >
  ...
  "계획 세워줘", "어떤 itda 스킬로 풀 수 있어?", "work-find 메모 받았는데 어떻게 진행해"처럼 말하면 됩니다.
...
work-find를 마치고 나면 후보 메모 한 장이 생긴다. 그런데 "그래서 이걸 어떻게 시작하지?"가 막힌다. 또는 처음부터 원하는 게 명확한데 — 예: "매주 거래처 메일 정리해서 슬랙에 요약 보내는 자동화" — 어떤 itda 스킬을 어떤 순서로 써야 하는지 모른다.
...
| 상황 | 맞는 스킬 |
|------|---------|
| 뭘 자동화할지 모르겠다 | work-find |
| 뭘 할지는 알고, 어떻게 진행할지 모른다 | work-plan (이 스킬) |
| work-find 메모 받은 후 다음 단계 | work-plan에 메모 첨부 |
...
호출 인자나 첫 메시지에 요구사항·메모가 이미 담겨 왔으면 입력 방식을 묻지 않는다 — 유형을 스스로 판별해(자유 텍스트 → Stage 2 / work-find 메모 → 추출 후 Stage 2 / 이전 work-plan 메모 → 재진입 모드) 바로 진행한다. 같은 세션에서 work-find 직후 이어받는 경우("이어서 계획 세워줘")도 파일 첨부를 요구하지 않는다 — 대화 컨텍스트의 인터뷰 결과·메모 내용을 그대로 입력으로 쓴다.

**빈손으로 시작했을 때만** AskUserQuestion 4지선다로 입력 방식을 확인한다:

1. (권장) 자유 텍스트로 요구사항을 그대로 적어주세요 — 1~2문장이면 충분합니다.
2. 직전에 만든 work-find 메모를 첨부했어요 (혹은 곧 첨부할게요).
3. 직전에 만든 work-plan 메모를 첨부했어요 (계획을 다듬고 싶어요).
4. 아직 정리가 안 됐어요 — work-find 스킬로 먼저 가는 게 낫겠어요.

옵션 4를 고르면 work-plan는 즉시 종료하고 work-find를 안내한다 (메모 미생성).

work-find 메모 첨부 시: 트랙(A/B/혼합), 문제 정의 한 줄, 관련 자료·도구에 더해 **가치 추정(§6)과 AI 출력 검증(§5)도 추출**해 Stage 2 입력으로 쓴다(`catalog_loader.extract_from_findwork_memo`). 가치 추정은 산출 메모 "요구사항" 말미에 한 줄로, AI 출력 검증(잘못된 출력을 알아채는 방법·검증 주기)은 "실패 시 대처"에 통합해 이월한다 — 계획이 요구하는 준비 비용(키 발급·자료 정리)을 정당화할 근거를 잃지 않는다.

이전 work-plan 메모 첨부 시: "어디를 다듬을지" 한 줄 입력을 받은 뒤, 그 부분만 Stage 2로 재진입한다. 변경하지 않는 섹션은 그대로 보존한다.

### Stage 2 — Mirror-back 정리 (핵심 단계)

[HARD] **이 단계는 어떤 경우에도 생략하지 않는다.** 사용자가 매우 상세하게 적어줘도 mirror-back은 수행한다. 단, **work-find 메모 첨부(또는 같은 세션 이어받기) 입력은 축약 mirror-back으로 진행한다** — 그 메모는 이미 work-find 인터뷰에서 합의된 산출물이므로 전체 4항목 재확인은 이중 노동이다. 메모 요지를 2~3줄로 요약하고 "메모 내용에서 달라진 점이 있나요?" 1문만 확인한다(생략이 아니라 축약 — 변경점이 있으면 해당 항목만 아래 일반 mirror-back으로 확인). 패턴은 `references/mirror-back-patterns.md` 참조.
...
- 옵션 1: 메모 파일을 `work-plan/YYYY-MM-DD_HHmm_<slug>.md`에 저장.
...
파일명: `work-plan/YYYY-MM-DD_HHmm_<slug>.md`

경로 결정: `resolve_data_dir("work-plan")` (shared/itda_path.py 위임)
...
## 요구사항
[사용자 입력 한 줄 + mirror-back 최종 합의 불릿 + 기대 가치 한 줄 (work-find 메모 §6 이월 또는 대화에서 파악한 절감 시간·산출물 가치, 파생 수치는 산출식 병기)]
...
## 실패 시 대처
[각 단계 실패 시나리오 + 한 줄 대응. AI가 잘못된 출력을 낼 때 알아채는 방법·검증 주기 포함 (work-find 메모 §5 이월)]
```
