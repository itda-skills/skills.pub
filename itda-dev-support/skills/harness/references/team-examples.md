...
```
[리더/오케스트레이터]
    ├── Agent ×4 병렬 스폰 (official·media·community·background — 암묵 팀 소속)
    ├── TaskCreate(4개 조사 작업)
...
Phase 2: 팀 구성
  - 단일 메시지에서 4개 Agent 호출로 팀원 병렬 스폰 (암묵 팀 소속):
      Agent(name: "official", prompt: "공식 채널 조사...")
      Agent(name: "media", prompt: "미디어/투자 동향 조사...")
      Agent(name: "community", prompt: "커뮤니티 반응 조사...")
      Agent(name: "background", prompt: "배경/경쟁 환경 조사...")
  - TaskCreate(tasks: [
...
- 세계관 변경 시 관련 팀원 각각에게 SendMessage로 전파
...
```
Phase 1: Agent 병렬 스폰 — worldbuilder + character-designer + plot-architect (단일 메시지 3개 호출)
...
Phase 2: Phase 1 팀원 작업 완료 확인 → prose-stylist를 서브 에이전트로 호출 (단독 집필이므로 팀 통신 불필요)
         prose-stylist가 _workspace/의 3개 산출물을 Read하여 집필
         → 결과를 _workspace/02_prose_draft.md에 저장

Phase 3: 새 팀원 스폰 — Agent(name: "science-consultant") + Agent(name: "continuity-manager") 병렬 호출
         (암묵 팀은 세션당 하나로 상시 존재 — 팀 생성·해체 없이 Phase마다 팀원 구성만 바뀐다)
