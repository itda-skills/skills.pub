...
사용 예:
  python3 collect_customs.py --limit 10
  python3 collect_customs.py --keyword 원산지 --pages 2
  python3 collect_customs.py --from 2026-07-01 --to 2026-07-27 --format json
  python3 collect_customs.py --xlsx notices.xlsx
"""
...
def render_markdown(rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        lines.append(f"| {r['date']} | {title} | {r['writer']} | [보기]({r['link']}) |")
    return "\n".join(lines)
...
def write_xlsx(rows: list[dict], path: str) -> None:
    ...
    ws = wb.active
    ws.title = "관세청 공지사항"
    ws.append(["날짜", "제목", "작성자", "조회수", "링크"])
    for r in rows:
        ws.append([r["date"], r["title"], r["writer"], r["views"], r["link"]])
    wb.save(path)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="관세청 공지사항 수집")
    ...
    try:
        for page in range(1, args.pages + 1):
            html = customs_api.fetch_list_html(page=page)
            rows.extend(customs_api.parse_list(html))
    except customs_api.CustomsError as exc:
