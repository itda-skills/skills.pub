...
description: >
  ...
  [책임 경계] 본 스킬은 용어집 기반 영업기밀 마스킹·왕복 복원 전담 — itda-privacy-gate:pii-redact 는 정형 PII 무상태 마스킹, itda-privacy-gate:synthetic-data 는 실제 데이터 없이 같은 구조의 가상 데이터 생성.
license: MIT
...
| 축 | `pii-redact` (itda-cs-analysis) | `biz-redact` (itda-privacy-gate) |
|---|---|---|
