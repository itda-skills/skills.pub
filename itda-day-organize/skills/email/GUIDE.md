...
네이버·Gmail·다음/카카오·아이클라우드·회사 메일(커스텀)에서 멀티 계정으로 메일을 보내고 받고 정리(폴더 이동·스팸·휴지통·읽음 처리)하는 스킬입니다. **Claude에게 자연어로 말하면 됩니다** — 터미널 명령을 직접 입력할 필요는 없습니다.

---

## 처음 설정하기

메일 서비스는 보안 때문에 계정 비밀번호를 그대로 쓰지 못합니다. **2단계 인증을 켠 뒤 "앱 비밀번호"를 따로 발급**받아, 그 값을 **`.env` 파일에 넣어 두면 (권장)** 스킬이 자동으로 찾아 읽습니다.
...
- **Claude 지침 (보조)** — Claude Desktop의 "Claude 지침"(설정 → 일반) 또는 Claude Code의 `CLAUDE.md`에 적는 방식도 동작하지만, 대화 컨텍스트에 값이 노출되므로 `.env`를 권장합니다.
...
4. `NAVER_EMAIL` / `NAVER_APP_PASSWORD`를 위 "처음 설정하기"의 **`.env` 파일**(또는 Claude 지침·`CLAUDE.md`)에 넣습니다.
...
4. `ICLOUD_EMAIL` / `ICLOUD_APP_PASSWORD`를 위 "처음 설정하기"의 **`.env` 파일**(또는 Claude 지침·`CLAUDE.md`)에 넣습니다. `@icloud.com`·`@me.com`·`@mac.com` 모두 동일하게 동작합니다.
...
```
/email 이 광고 메일 스팸 처리하고, 지난주 뉴스레터는 휴지통으로 보내줘
```
...
먼저 `/email 네이버 work 계정 폴더 목록 보여줘`로 폴더 이름을 확인한 뒤 정확한 이름으로 다시 요청하면 됩니다.

### 받은편지함 정리하기 (이동·폴더·스팸·휴지통·읽음)

쌓인 메일을 자연어로 정리할 수 있습니다. 폴더를 만들어 옮기고, 스팸을 치우고, 필요 없는 메일은 휴지통으로 보내세요.

```
/email "업무" 폴더 만들어줘
/email 김대리가 보낸 이 메일 업무 폴더로 옮겨줘
/email 이거 스팸 처리해줘
/email 이 메일 스팸 아니야, 받은편지함으로 돌려줘
/email 이 광고 메일들 휴지통으로 보내줘
/email 어제 휴지통에 넣은 메일 복원해줘
/email 이 메일들 읽음 처리해줘
/email 옛프로젝트 폴더 이름을 "2025 아카이브"로 바꿔줘
```

폴더 이름은 한글·공백이 있어도 그대로 말하면 됩니다. 받은편지함·보낸편지함·휴지통·스팸함 같은 **기본 폴더는 이름 변경·삭제가 되지 않도록 보호**되어 있고, 내용물이 있는 폴더를 삭제할 때는 안의 메일을 휴지통으로 옮긴 뒤 지울지 먼저 물어봅니다.

#### 메일 정리 주의사항

- **영구 삭제는 두 번 확인합니다** — "휴지통 비워줘", "영구 삭제해줘"처럼 명시적으로 요청했을 때만 실행되며, 그 전에 Claude가 **삭제될 메일 목록(보낸이·제목)을 먼저 보여주고 확인을 받은 뒤** 진행합니다. 영구 삭제된 메일은 복구할 수 없습니다.
- **스팸 처리는 폴더 이동입니다** — 메일을 스팸함으로 옮겨 주지만, 메일 서비스의 스팸필터가 그것을 학습해 비슷한 메일을 자동 차단해 준다는 보장은 없습니다. 반복 수신이 문제라면 메일 서비스의 차단/필터 설정을 함께 쓰세요.
- **Gmail은 폴더가 아니라 라벨** — Gmail에서 "폴더 이동"은 라벨을 바꾸는 동작입니다. 한 메일에 라벨을 여러 개 붙이는 것은 이 스킬로는 되지 않습니다.
- **스팸 해제·휴지통 복원은 받은편지함으로** — 원래 있던 폴더가 아니라 항상 받은편지함으로 돌아옵니다. 다른 폴더로 두고 싶으면 복원 후 "업무 폴더로 옮겨줘"라고 이어서 요청하세요.
...
`/email 받은편지함 새 메일 3건 보여줘`라고 요청하면 아래처럼 정리됩니다.
...
- **계정 정보는 `.env` 파일에 (권장)** — 작업 폴더(Cowork 연결 폴더 / Claude Code 프로젝트 루트) 루트에 `.env` 파일을 만들어 이메일·앱 비밀번호를 넣어 두면 자동으로 읽힙니다. 점(`.`)으로 시작하는 파일을 만들기 어렵다면 **`환경변수.txt`** 라는 이름으로 만들어도 똑같이 읽힙니다(메모장이 `.txt` 를 붙여 `.env.txt` 가 되어도 됩니다). Claude Desktop의 "Claude 지침"(설정 → 일반)이나 Claude Code의 `CLAUDE.md`에 적는 방식도 동작하지만, 대화 컨텍스트에 값이 노출되므로 `.env`를 권장합니다.
...
- **"메일을 보낼 수 없다 / 연결이 끊겼다"** — 가장 흔한 경우입니다. 스킬이 자동으로 다른 경로로 재시도하고, 그래도 안 되면 메일을 **잠시 보관**합니다(응답에 "queued" 표시). 이때는 본인 PC 환경에서 `/email 보관된 메일 보내줘`라고 하면 발송됩니다.
- **로컬에서는 되는데 Cowork에서만 실패** — 거의 네트워크 정책 문제입니다. `/email 메일 진단해줘`라고 하면 어느 단계(인터넷 연결·발송 경로·계정 인증)에서 막혔는지 알려줍니다.
